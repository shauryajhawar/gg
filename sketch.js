...
function setup() {
  ...
  invg = createSprite(200,height/2+500, 400, 5);
  ...
  restart = createSprite(width / 2 + 400, height / 2 - 800,10, 10);
...
function spawnObstacles() {
  ...
  if (frameCount % 200 === 0) {
    stone = createSprite(610, height/2 + 493, 10, 10);
    stone.x = 610;
...
function spawnBanana() {
  if (frameCount % 120 === 0) {
    ...
    banana.x = 610;
    banana.y = random(height-705, height  - 755);
...
function SpawnGround() {
  if (frameCount % 140 === 0) {
    uphill = createSprite(610, height - 645);
    uphill.x = 610;
